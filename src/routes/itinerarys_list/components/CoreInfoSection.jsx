...
const CoreDetailsSection = ({ formData, handleInputChange, styles, isEditing }) => {
  const { cardStyle, labelStyle, inputStyle } = styles;

  const themes = [
    "Family", "Honeymoon", "Adventures", "Solo", "Wildlife",
    "Beach", "Pilgrimage", "Hill Station", "Heritage Tour",
    "Ayurveda Tour", "Cultural Tour", "Luxury Tour", "Budget Tour",
    "Family Tour", "Bachelor Tour", "Women Group", "Special Interest",
  ];
  ...
  const handleClassificationChange = (e) => {
    ...
    handleInputChange({ target: { name: "classification", value: updatedClassification } });
  };
  ...
  return (
    <div className={cardStyle}>
      <h2 className="mb-4 border-b border-gray-700 pb-2 text-xl font-semibold flex items-center">
        <Layers className="text-primary mr-2" size={20} /> Core Details
      </h2>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        ...
        <div>
          <label className={labelStyle}>
            <Text className="text-muted-foreground mr-2 inline" size={16} /> Title
          </label>
          {isEditing ? (
            <input
              type="text"
            ...
            <p className="text-gray-800 dark:text-gray-200">{formData.title || "-"}</p>
          )}
        </div>
        ...
        <div>
          <label className={labelStyle}>
            <Globe className="text-muted-foreground mr-2 inline" size={16} /> Travel Type
          </label>
          {isEditing ? (
            ...
            <p>{formData.travel_type || "-"}</p>
          )}

          <label className={`${labelStyle} mt-4 block`}>
            <MapPin className="text-muted-foreground mr-2 inline" size={16} /> Destination
          </label>
          {isEditing ? (
            <select
            ...
            <p>{formData.selected_destination_name || "-"}</p>
          )}
        </div>
        ...
        <div>
          <label className={labelStyle}>
            <Calendar className="text-muted-foreground mr-2 inline" size={16} /> Duration
          </label>
          {isEditing ? (
            <select
              name="duration"
            ...
            >
              ...
              {["2 Days / 1 Night","3 Days / 2 Nights","4 Days / 3 Nights","5 Days / 4 Nights","6 Days / 5 Nights","7 Days / 6 Nights","10 Days / 9 Nights","Custom"].map((dur) => (
          ...
          {isEditing && formData.duration === "Custom" && (
        ...
        <div>
          <label className={labelStyle}>
            <ListChecks className="text-muted-foreground mr-2 inline" size={16} /> Theme
          </label>
          {isEditing ? (
            ...
            <p>{formData.itinerary_theme?.join(", ") || "-"}</p>
          )}
        </div>
        ...
        <div>
          <label className={labelStyle}>
            <ListChecks className="text-muted-foreground mr-2 inline" size={16} /> Classification
          </label>
          {isEditing ? (
            <div className="mt-2 flex flex-wrap gap-3">
            ...
            <p>{formData.classification?.join(", ") || "-"}</p>
          )}
        </div>
        ...
        <div>
          <label className={labelStyle}>
            <Layers className="text-muted-foreground mr-2 inline" size={16} /> Type
          </label>
          {isEditing ? (
            <select
              name="itinerary_type"
        ...
        <div>
          <label className={labelStyle}>
            <Eye className="text-muted-foreground mr-2 inline" size={16} /> Visibility
          </label>
          {isEditing ? (
            <select
            ...
            <p>{formData.itinerary_visibility || "-"}</p>
          )}
        </div>
      </div>
    </div>
